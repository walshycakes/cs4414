...
int main(int argc, char *argv[]) {
  ...
  uthread_create(foo, 2, 1);
  uthread_create(foo, 3, 1);
...
void foo(int th) {
  ...
  if (local == test) {
    printf("foo - thread %i: PASSED\n", th);
  } else {
    printf("foo - thread %i: FAILED\n", th);
  }
...
void bar(int th) {
  ...
  if (local == test) {
    printf("bar - thread %i: PASSED\n", th);
  } else {
    printf("bar - thread %i: FAILED\n", th);
  }
